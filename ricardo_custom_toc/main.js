...
define([
    ...
    'nbextensions/ricardo_custom_toc/ricardo_custom_toc'
], function(
    requirejs,
    $,
    Jupyter,
    ...
    // imports
    var highlight_toc_item = ricardo_custom_toc.highlight_toc_item;
    var table_of_contents = ricardo_custom_toc.table_of_contents;
    var toggle_toc = ricardo_custom_toc.toggle_toc;
    var IPython = Jupyter;
            ...
            if ($('#save_html_with_toc').length == 0) {
                ...
                $('#save_html_with_toc')
                    ...
                    .on('click', function (evt) {
                        if (IPython.notebook.metadata.kernelspec.language == 'python') {
                            var code = "!jupyter nbconvert '" + IPython.notebook.notebook_name + "' --template ricardo_custom_toc";
                            console.log('[ricardo_custom_toc] running:', code);
    ...
    var toc_init = function() {
        ...
        IPython.notebook.config.loaded.then(function () {
            var cfg = ricardo_custom_toc.read_config();
    ...
    var load_ipython_extension = function() {
        ...
        if (Jupyter.notebook !== undefined && Jupyter.notebook._fully_loaded) {
            ...
            console.log("[ricardo_custom_toc] Notebook fully loaded -- ricardo_custom_toc initialized ")
            toc_init();
        } else {
            console.log("[ricardo_custom_toc] Waiting for notebook availability")
            events.on("notebook_loaded.Notebook", function() {
                console.log("[ricardo_custom_toc] ricardo_custom_toc initialized (via notebook_loaded)")
                toc_init();
            })
        }
